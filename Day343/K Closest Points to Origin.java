...
Given an integer k and an array of points points[][],
where each point is represented as points[i] = [xi, yi] on the X-Y plane. Return the k closest points to the origin (0, 0).
...
class Solution {
    public ArrayList<ArrayList<Integer>> kClosest(int[][] points, int k) {
        ...
        while(!maxHeap.isEmpty()){
            ...
            int[] p = points[top[1]];
            res.add(new ArrayList<>(Arrays.asList(p[0], p[1])));
        }
