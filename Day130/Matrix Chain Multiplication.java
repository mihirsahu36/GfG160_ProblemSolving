...
There are two ways to multiply: ((M1 x M2) x M3) and (M1 x (M2 x M3)), note that the result of (M1 x M2) is a 2 x 3 matrix and result of (M2 x M3) is a 1 x 4 matrix. 
...
Let this 4 input matrices be M1, M2, M3 and M4. The minimum number of multiplications are obtained by ((M1 x M2) x M3) x M4). The minimum number is (1 x 2 x 3) + (1 x 3 x 4) + (1 x 4 x 3) = 30.
...
class Solution {
    static int solve(int []arr, int i, int j, int [][]t){
        ...
        for(int k=i;k<j;k++){
            int steps = solve(arr, i, k, t) + solve(arr, k + 1, j, t) + (arr[i-1] * arr[k] * arr[j]);
            minSteps = Math.min(minSteps, steps);
        }
        
        return t[i][j] = minSteps;
    }
