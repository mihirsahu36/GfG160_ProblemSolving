...
class Solution {
    static ArrayList<Integer> subarraySum(int[] arr, int target) {
        ...
        while (end < n) {
            ...
            while (sum > target && start <= end) { // shrink the window from the left if the sum exceeds the target
                sum -= arr[start];
                start++;
            }

            if (sum == target) {
                result.add(start + 1); // start index
                result.add(end + 1);   // end index
                return result;
            }

            end++; // move the end pointer to next element
        }
